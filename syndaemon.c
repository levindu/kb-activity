...
/* CONFIG */
#define HAVE_X11_EXTENSIONS_RECORD_H
/* End of CONFIG */
...
{
    fprintf(stderr,
            "Usage: syndaemon [-i idle-time] [-m poll-delay] [-d] [-t] [-k] <toggle touchpad command>\n");
    fprintf(stderr,
...
main_loop(Display * display, double idle_time, int poll_delay)
{
    ...
    for (;;) {
		...
		printf("current_time = %.2lf, last_active = %.2lf\n", current_time, last_activity);
        if (current_time > last_activity + idle_time + 0.05) { /* Enable touchpad */
            toggle_touchpad(True);
        }
...
main(int argc, char *argv[])
{
	...
	if (optind >= argc)
		usage();
	c = strlen(argv[optind]);
	command = malloc(c + 3);
	strcpy(command, argv[optind]);
	strcat(command, " 1");
	command_parameter = command + c + 1;
